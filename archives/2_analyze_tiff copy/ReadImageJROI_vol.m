function [sROI, matROIs, zPos] = ReadImageJROI_vol(cstrFilenames, shape)
...
if (iscell(cstrFilenames))
   ...
   cvsROI = cellfun(@ReadImageJROI_vol, CellFlatten(cstrFilenames), 'UniformOutput', false);
...
if (isequal(lower(strExt), '.zip'))
   ...
   cvsROIs = ReadImageJROI_vol(cstrFilenames);
   ...
   zPos = zeros(numel(cvsROIs),1);
%    try
   for num_comp = 1:numel(cvsROIs)
       cmp = cvsROIs{num_comp};
       zPos(num_comp) = cmp.nPosition;
       coords = cmp.mnCoordinates;
       
       % fix large ROI issue (which raises error in sub2ind())
       if max(coords(:,2)) > shape(1)  % if roi excede y boundary
           maxY = max(coords(:,2));  % fix it
           roi_excede_y = maxY-shape(1);
           coords(coords(:,2)>shape(1),2)= shape(1);
           if roi_excede_y>2
               fprintf('roi%d excedes Y max by %d pix\n',num_comp,round(roi_excede_y))
           end
       end

      % fix large ROI issue (which raises error in sub2ind())
       if max(coords(:,1)) > shape(2)  % if roi excede x boundary
           maxX = max(coords(:,1));  % fix it
           roi_excede_x = maxX-shape(2);
           coords(coords(:,1)>shape(2),1)= shape(2);
           if roi_excede_x>2
               fprintf('roi%d excedes X max by %d pix\n',num_comp,round(roi_excede_x))
           end
       end
       % fix small ROI issue (which raises error in sub2ind())
       if min(min(coords)) < 1
           coords(coords<1)= 1;
       end
       ...
       mm = zeros(shape);
       mm(int16(coords_sqr)) = 1;
       mm = bwconvhull(mm);
       matROIs(:,:,num_comp) = mm;
   end   
%    catch
%        disp('an ROI cannot be read');
%    end
   return ;
end
