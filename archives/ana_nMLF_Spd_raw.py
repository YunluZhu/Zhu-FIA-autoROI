    ...
    for repeat in repeats:
        if repeat[0] != '.':
            ...
            for sti in np.arange(4):
                sti += 1
                load_raw_thisSti = scipy.io.loadmat(f'{exp_path}/raw{sti}.mat')
                this_repres = load_raw_thisSti[f'res{sti}']
                ...
                raw = pd.concat([raw, this_repres_output])
              
# %%
raw['unilateral'] = 'l'
raw.loc[raw['roi']>=8, 'unilateral'] = 'r'
volume_rate = 11.171 / 9
raw['time'] = raw['frame']/volume_rate
sti_map = {
    1:5,
    2:10,
    3:20,
...
raw['pitch'] = raw['stimulus'].map(sti_map)
set_font_type()
...
p = sns.relplot(
    kind='line',
    data=raw.query("condition == 'pre' & stimulus > 1"),
    x='time',
    y='raw',
    hue='pitch',
    aspect=1,
...
filename = "raw nMLF to nose-up pitch time series.pdf"
plt.savefig(filename,format='PDF')
# %%
# get peak
how_many_frames = 2
raw_get_peak = raw.query("frame < @how_many_frames")
raw_amplitude = raw_get_peak.groupby(['condition','pitch','repeat','unilateral']).mean().reset_index()
...
filename = "raw nMLF Tunings post lesion unilateral raw.pdf"
plt.savefig(filename,format='PDF',)
# %
g = sns.relplot(
    data=raw_amplitude,
...
filename = "raw nMLF Tunings post lesion unilateral.pdf"
plt.savefig(filename,format='PDF',)
