...
NOTE slope on raw data with repeats
-----todo-----
...
# first condition/area
df_peak_time = df_peak_time.loc[df_peak_time.area.isin([1])]

# categorize responses by their peak time in selected two stimulus
df_peak_time_cat = df_peak_time.loc[df_peak_time.nsti.isin([1,2,3,4])].groupby(['fish_id','exp_cond','area','ROI'])['peak_time'].median().reset_index()
...
slope_chg = slope_chg.assign(
    slope_chg = slope_cond2['slope_rawAmp'].values - slope_cond1['slope_rawAmp'].values,
    slope_chg_ratio = (slope_cond2['slope_rawAmp'].values - slope_cond1['slope_rawAmp'].values)/slope_cond1['slope_rawAmp'].values
)
...
slope_threshold = np.percentile(amp_goodFit_SlopeOnAveragedAmp.slope_rawAmp, 80)
print(f"Slope thresholded {slope_threshold}")
ROI_goodTuning = amp_goodFit_SlopeOnAveragedAmp.query("cond_num == 1 & slope_rawAmp > @slope_threshold").ROI.values
...
# sns.lineplot(data=ppdf, x='cond_num', y='slope_rawAmp')
#%%
