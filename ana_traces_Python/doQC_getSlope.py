...
def doQC_getSlope(root):
    ...
    sel_fitted_forQC1 = amp_QC.loc[amp_QC['cond_num'].isin([1,2]),:].groupby('ROI_id')['rLowAng'].min().reset_index()
    ROI_goodFit1 = sel_fitted_forQC1.query("rLowAng > @r_threshold").ROI_id.unique()
    ...
    ROI_passTuning = sel_fitted_forQC.groupby(['ROI_id'])['slopeLowAng'].max() > slope_threshold
